...
const OrderUI = () => {
    ...
    const getOrders = async () => {
        let url = "http://localhost:5000/api/order/orders/filter?";
        ...
        if (queryParams.length > 0) {
            ...
            url = "http://localhost:5000/api/order/orders/filter";
        }
    ...
    const deleteOrder = async ({ itemId, orderId }) => {
        try {
          const res = await axios.delete(`http://localhost:5000/api/order/orders/${orderId}/${itemId}`);
          console.log(res);
          await getOrders();
        } catch(err){
            console.log(err)
                        ...
                        <tr  key={`${order._id}-${itemIndex}`}>
                            <td>
                                <div className='transactionOwner'>
                                    <img className='transactionOwner-Img' src={`http://localhost:5000${item?.productId?.image1}`} alt='product'/>
                                </div>
                            </td>
                                ...
                                {(() => {
                                    ...
                                    switch (item.status) {
                                        ...
                                        case 'REF':
                                            className = 'statusREF';
                                            svgContent = (
                                                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-x"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg>
                                            );
                                            break;
                                        case 'RTR':
                                            className = 'statusRTR';
                                            svgContent = (
                                                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-corner-down-left"><polyline points="9 10 4 15 9 20"></polyline><path d="M20 4v7a4 4 0 0 1-4 4H4"></path></svg>
                                            );
                                            break;
                                        case 'LVR':
                                            className = 'statusLVR';
